-- Cache auth/helper function checks once per statement in Team access RLS policies.
...
to authenticated
using (
  (select app_private.is_dashboard_owner())
  or (
    (select app_private.is_team_access_user())
    and active = true
...
using (
  (select app_private.is_dashboard_owner())
  or id = (select app_private.current_team_member_id())
);
...
using (
  (select app_private.is_dashboard_owner())
  or team_member_id = (select app_private.current_team_member_id())
);
...
with check (
  (select app_private.is_dashboard_owner())
  or (
    team_member_id = (select app_private.current_team_member_id())
