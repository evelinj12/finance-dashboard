-- Phase 1 finance dashboard repair.
-- Adds source-tracking fields needed by the 2025-2026 import and a monthly
-- summary view for dashboard rollups.
...
alter table income_sources
  add column if not exists source_key text,
  add column if not exists visible_in_active_breakdown boolean not null default true;

update income_sources
set visible_in_active_breakdown = true
where visible_in_active_breakdown is null;

alter table income_sources
  alter column visible_in_active_breakdown set default true,
  alter column visible_in_active_breakdown set not null;
...
update income_sources
set name = case name
    when 'Client 6 (since Jan 2025)' then 'Agent EA'
    when 'Client 7 (since Jun 2025)' then 'Erica - BCC'
    when 'Client 8 (since Jun 2026)' then 'JML Media'
    when 'Client 11 (since Jan 2024)' then 'Jasper'
    when 'Client 13 (since Jan 2024)' then 'Z PD'
    else name
  end,
  active = true,
...
where lower(name) in (
  ...
  'z pd',
  'zpd',
  'client 6 (since jan 2025)',
  'client 7 (since jun 2025)',
  'client 8 (since jun 2026)',
  'client 11 (since jan 2024)',
  'client 13 (since jan 2024)'
);
...
alter table contractor_payments
  add column if not exists client_or_project text,
...
begin
  if not exists (
    select 1
    from pg_constraint
    where conname = 'contractor_payments_status_check'
      and conrelid = 'contractor_payments'::regclass
  ) then
    alter table contractor_payments
      add constraint contractor_payments_status_check
      check (status in ('owed', 'paid', 'transferred', 'unknown'));
  end if;
end $$;
...
alter table net_worth_snapshots
  ...
  add column if not exists source_sheet text,
  add column if not exists source_row text,
  add column if not exists breakdown_quality text not null default 'full';

update net_worth_snapshots
set snapshot_date = month
where snapshot_date is null;

update net_worth_snapshots
set breakdown_quality = 'full'
where breakdown_quality is null;

alter table net_worth_snapshots
  alter column breakdown_quality set default 'full',
  alter column breakdown_quality set not null;
...
begin
  if not exists (
    select 1
    from pg_constraint
    where conname = 'net_worth_snapshots_breakdown_quality_check'
      and conrelid = 'net_worth_snapshots'::regclass
  ) then
    alter table net_worth_snapshots
      add constraint net_worth_snapshots_breakdown_quality_check
      check (breakdown_quality in ('full', 'total_only'));
  end if;
end $$;
...
final as (
  select
    ...
    -- Phase 1 invariant: contractor_payments is canonical for Kevin/Punya Kev
    -- payout impact here; imports must not also load those payouts as normal
    -- fixed, variable, or sinking-fund transaction outflows.
...
select
  ...
  case
    ...
    else (sinking_funds_idr + greatest(net_after_savings_idr, 0)) / total_income_idr
